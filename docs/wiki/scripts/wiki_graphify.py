...
Wiki `_meta/GRAPH.yaml` is a compiled *claim* index from page frontmatter only.

  python3 docs/wiki/scripts/wiki_graphify.py sync
  python3 docs/wiki/scripts/wiki_graphify.py query "who calls TypeChecker"
  ...
  python3 docs/wiki/scripts/wiki_graphify.py export-wiki
"""
...
from wiki_paths import ROOT, load_host
...
def cmd_passthrough(verb: str, rest: list[str]) -> None:
    ...
    path = graph_json_path(cfg)
    if not path.exists():
        raise SystemExit(f"No {path}. Run: python3 docs/wiki/scripts/wiki_graphify.py sync")
    args = [verb, *rest]
