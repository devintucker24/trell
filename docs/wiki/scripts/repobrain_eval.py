...
def evaluate_doctor(root: Path) -> CategoryResult:
    script = root / "docs" / "wiki" / "scripts" / "wiki_doctor.py"
    proc = run([sys.executable, str(script), "--no-log"], cwd=root)
    report_path = root / "docs" / "wiki" / "_meta" / "doctor-latest.json"
    ...
    if proc.returncode != 0 or not report_path.exists():
        return CategoryResult(
            ...
            evidence,
            ["Run `python3 docs/wiki/scripts/wiki_doctor.py` and fix the reported error."],
        )
    ...
    return CategoryResult(
        ...
        (
            ...
            if blocking == 0
                ...
                "Run `python3 docs/wiki/scripts/wiki_doctor.py`, then apply each "
...
def retrieve_case(
    ...
    args = [
        sys.executable,
        str(root / "docs" / "wiki" / "scripts" / "wiki_retrieve.py"),
        str(case["q"]),
        "--k",
        str(top_k),
        "--budget-tokens",
    ...
    command = (
        f'python3 docs/wiki/scripts/wiki_retrieve.py "{case["q"]}" '
    ...
    for case in config.get("answer_fidelity") or []:
        ...
        context_manifest = [
            ...
            for path in (config.get("tier0") or {}).get(
                "paths", ["AGENTS.md", "docs/wiki/ROUTER.md"]
            )
        ] + [
            {
...
def evaluate_budgets(
    ...
    paths = tier0.get("paths") or ["AGENTS.md", "docs/wiki/ROUTER.md"]
    path_costs = []
    missing = []
    for rel in paths:
        path = root / rel
...
def evaluate_graphify(root: Path, config: dict[str, Any]) -> CategoryResult:
    ...
    targets = (graph_cfg.get("targets") or ["src"]) if root == ROOT else ["src"]
    ...
    return CategoryResult(
        ...
        (
            ...
            if not failures
                ...
                "run `python3 docs/wiki/scripts/wiki_graphify.py sync`, then rerun eval."
            ]
        ),
    )
...
def evaluate_setup_fixture(root: Path) -> CategoryResult:
    with tempfile.TemporaryDirectory(prefix="repobrain-eval-") as tmp:
        ...
        export = run(
            [
                sys.executable,
                str(root / "docs" / "wiki" / "scripts" / "wiki_pack.py"),
                "export",
                str(fixture),
            ],
            cwd=root,
        ...
        setup = run(
            [
                sys.executable,
                str(fixture / "docs" / "wiki" / "scripts" / "wiki_setup.py"),
                "--no-graphify",
            ],
            cwd=fixture,
        )
