...
agent:
  ...
  maintain:
    - "Keep vocabulary in sync with OPERATOR.md graph sections"
---
...
**Engine vs host:** schema and memory lanes are portable. Domain folders live
in `_system/config/HOST.yaml`. See [[_system/docs/FRAMEWORK]].
...
## 4. Graphs (machines only)

Two graphs. Neither is a human document. Do not paste them into chat.

### 4.1 Code graph — Graphify (source of truth for structure)
...
Do not maintain a second AST/call graph in this pack.

### 4.2 Claim graph — compiled from wiki frontmatter

Canonical aggregate: `docs/wiki/_system/generated/claim-graph.yaml`.

This is **not** a rival to Graphify. It is an index of typed *claims* on pages (`reduces_via`, `contradicts`, …) so retrieve can do one-hop proximity without dumping every page.
...
Protocol: `docs/wiki/_system/docs/GRAPH.md`.
...
These exist under `docs/wiki/_system/`; doctor/retrieve skip the entire engine:

| Folder | Role |
|--------|------|
| `skills/` | Canonical RepoBrain playbooks |
| `scripts/` | Python operators and path resolver |
| `templates/` | Setup/export inputs |
| `generated/` | Machine reports and claim index |
| `host/` | Host overlay pages (this instance: `router-seeds.md`) — **are** wiki pages |
| `HOST.yaml` | Project overlay (not markdown) |

Keep third-party skills outside `_system/skills/`; harness directories contain adapters.
