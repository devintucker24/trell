---
name: repobrain-usage
description: Log and score RepoBrain usage — JSONL events, retrieve token estimates, hit quality, doctor score, dump admissions, usefulness index. Use after retrieve/doctor, when tuning budgets, or when the user asks for a usage dashboard.
---

# Skill: RepoBrain Usage (telemetry)

## When to use
- After a retrieve/query session (scripts already append events)
- User asks “RepoBrain usage” or the historical phrases “is the wiki earning its context cost?”, “usage dashboard”, “token spend”
...
### Automatic
`./repobrain retrieve` and `./repobrain doctor` append JSONL unless `--no-log`.

### Manual / agent protocol
```bash
# Query turn: pages you opened vs pages you cited
./repobrain usage log --op query \
  --query "<question>" \
...
./repobrain usage log --op dump --query "<why>"

# Snapshot
./repobrain dashboard usage --days 30
```

### Scoring
`./repobrain dashboard usage` writes the dashboard and prints JSON including **usefulness index** (0–100): hit quality + last doctor score + activity − dump penalty.
