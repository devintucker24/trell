...
1. Confirm `docs/wiki/HOST.yaml` `anchor` (one paragraph the wiki must not dilute)
...
From a repo that **already has** `docs/wiki/SCHEMA.md` (this pack):
...
Equivalent: `python3 docs/wiki/scripts/wiki_pack.py setup`
...
| `_meta/GRAPH.yaml` | **Yes** — compiled from page frontmatter | `sync_graph.py` |
...
4. `python3 docs/wiki/scripts/wiki_doctor.py`
5. `python3 docs/wiki/scripts/wiki_retrieve.py "what is this repo" --budget-tokens 1500`

Code questions: `python3 docs/wiki/scripts/wiki_graphify.py query "…"`

Need Graphify: `pip install graphifyy` (CLI name is still `graphify`).

Operator: `docs/wiki/FRAMEWORK.md`  
Claim vs code graphs: `docs/wiki/_meta/GRAPH.md`
