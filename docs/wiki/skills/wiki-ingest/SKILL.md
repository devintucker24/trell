...
## Prerequisite
If material is still a chat paste / URL / unknown blob → run **`wiki-triage`** first (or drop into `docs/wiki/inbox/`).
...
Use `wiki-maintain` matrix; still log as `maintain` or `ingest`.

### C. Always finish with
1. `python3 docs/wiki/scripts/sync_graph.py`
