...
- Query: `graphify query|path|explain|affected|god-nodes` via `docs/wiki/scripts/wiki_graphify.py`.
...
Canonical aggregate: `docs/wiki/_meta/GRAPH.yaml` (generated by `sync_graph.py`).
...
After editing page `nodes`/`edges`, run `python3 docs/wiki/scripts/sync_graph.py`.

Protocol for agents: `docs/wiki/_meta/GRAPH.md`.
...
| `scripts/` | Python operators (`retrieve`, `doctor`, `setup`, `graphify` wrapper, …) |
