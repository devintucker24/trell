---
name: wiki-doctor
description: Diagnose Trell wiki health without changing files — frontmatter, graph orphans, broken links, stale inbox, schema drift. Use when user asks for wiki doctor, audit, diagnosis, or before heal.
---
...
```bash
python3 skills/wiki/scripts/wiki_doctor.py
```
This writes:
...
### 5. Handoff
- If findings → run **`skills/wiki/heal`** with the doctor report as input
- If clean → log only; do not churn files

## Anti-patterns
- Doctor rewriting pages "while we're here" → use **heal**
- Doctor inventing new domains to "fix" orphans → SCHEMA gate
- Skipping the script and hand-waving → always run `wiki_doctor.py` first
