...
Writes:
  docs/wiki/_meta/doctor-YYYY-MM-DD.md
  docs/wiki/_meta/doctor-latest.json
"""
...
def main() -> None:
    ...
    # Graph
    graph_path = META / "GRAPH.yaml"
    if not graph_path.exists():
        add(findings, "critical", "no_graph", "GRAPH.yaml missing", "_meta/GRAPH.yaml", "sync_graph.py")
        graph = {"nodes": [], "edges": []}
    else:
    ...
    latest = META / "doctor-latest.json"
    latest.write_text(json.dumps(report, indent=2), encoding="utf-8")

    md_path = META / f"doctor-{TODAY}.md"
    lines = [
        "---",
        f"id: doctor-{TODAY}",
